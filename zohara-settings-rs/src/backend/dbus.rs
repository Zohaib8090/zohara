...
/// Get the current Wi-Fi enabled state from NetworkManager via D-Bus
pub async fn nm_wifi_enabled() -> Result<bool, zbus::Error> {
...
pub async fn nm_set_wifi(enabled: bool) -> Result<(), zbus::Error> {
    let conn = Connection::system().await?;
    let proxy = zbus::Proxy::new(
        &conn,
        "org.freedesktop.NetworkManager",
